...
/**
 * 这个类用来表示约束关系中的 constraints.TGD，其形式为：body -> head
 * 其中 body和 head分别由一个或多个 symbols.RelationalAtom 组成，RelationalAtom之间用"and"分隔
...
public class TGD extends Constraint {
    ...
    /**
     * 对于某一条 constraints.TGD，获取它在数据库下的所有 Trigger
     *
    ...
    public List<Trigger> getTriggers(Database database) {
        ...
        if (body.size() == 1) { // 首先处理LinearTGD的情况，只需考虑body中一个表，得到Variable和Value的映射关系
            RelationalAtom bodyAtom = body.get(0);
...
/*
TODO:1.对于body中有多个Atom的 TGD，怎样得到它对应的 trigger?
       是否需要自己实现多表联合查询(自然连接)?如何记住连接属性?
       ————修改完善 getTriggers()方法
 */
